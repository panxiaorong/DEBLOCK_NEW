...
{
	float max = Dual_Mode_Deblock_Learner::Max(); //8
	float min = Dual_Mode_Deblock_Learner::Min(); //8
	Update_a3_k();
	int F = Compute_F();
...
{
	int width_step = single_channel.cols - 1;
	int height_step = (single_channel.rows - 8) / 8;
	for (int i = 1; i <= height_step; i++)
	{
		for (int j = 0; j <= width_step; j++)//j++ j = j+2
		{			
			int x_start = 8 * i - 1;
			int y = j;
...
{
	int height = updated_single_channel_y.rows;
	int width = updated_single_channel_y.cols;
	...
	int width_step = (single_channel.cols - 8) / 8;
	int height_step = single_channel.rows - 1;

	for (int i = 0; i <= height_step; i++)//i = i+2  i++
	{
		for (int j = 1; j <= width_step; j++)
		{
			int x = i;
			int y_start = 8 * j - 1;
